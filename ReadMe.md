...
Most text-to-SQL systems are a single LLM call: prompt in, SQL out. IntelliQ is a **stateful, multi-step agent** — each query passes through a graph of specialised nodes that reason, route, decide, and learn from prior interactions.
...
Built with [LangGraph](https://github.com/langchain-ai/langgraph) — a framework for building stateful, graph-structured LLM agents. The agent maintains conversation memory across turns via a SQLite checkpointer, enabling natural follow-up queries like _"now filter that by North region"_ without re-stating context.
...
A key design decision: instead of separate "check cache" and "classify query" nodes (each costing a rate-limited LLM call), IntelliQ fuses both decisions into a single structured output call (`CacheClassifyOutput`). The model simultaneously decides cache status AND query type, saving one LLM call on every request.
